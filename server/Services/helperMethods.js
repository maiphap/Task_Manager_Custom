...
const validateCardOwners = async (card = null, list, board, user, isCreate = false) => {
	const validate = isCreate ? true : list.cards.some((item) => item.toString() === card._id.toString());
	const validate2 = board.lists.some((item) => item.toString() === list._id.toString());
	const validate3 = user.boards.some((item) => item.toString() === board._id.toString());
